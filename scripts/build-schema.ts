...
const CustomRuleSchema = z
	.strictObject({
		name: z
			...
			.describe(
				"Unique identifier for the rule (case-insensitive for duplicate detection)",
			),
		command: z
			.string()
		...
		reason: z
			...
			.min(1)
			.max(256)
			.describe("Message shown when the command is blocked"),
	})
...
const ConfigSchema = z.strictObject({
	...
	rules: z
		...
		.default([])
		.describe("Custom blocking rules"),
});
